 ...
 *  雪花算法对象。
 *  @constructor
 *  @param {nuber} funScene - 应用场景
 *  @returns {string}
 */
function Snowflake (funParam) {
    this.scene = funParam.scene                 // 应用场景区分数值。
    this.worker = funParam.worker               // 机器标识。
    this.sequence = 0                           // 序列记录。
    this.launchTime = funParam.launch           // 起始时间，时间戳格式。秒单位：1234567890，毫秒单位：1234567890000。
    this.createdTime = funParam.created         // 最后创建时间，用于对比是否同一时间。
    this.modifiedTime = funParam.modified       // 本地时间发生回滚时的补偿时间。
...
const output = function (funLaunchTime, funScene) {
    return new Snowflake (funLaunchTime, funScene)
}
